...
\usepackage{esint, enumitem, graphicx}
...
\begin{abstract}
We develop structured numerical linear algebra algorithms for constructing symmetric doubly stochastic matrices with prescribed Sule\u{\i}manova spectrum.  The algorithms use the Schur realisation
\(P(\Lambda)=Q\Lambda Q^{\top}\), where \(Q\) is a structured orthogonal basis with Perron column \(\mathbf 1/\sqrt n\); entrywise non-negativity is certified by a coherence constant that is cheap to compute and sharp for the bases considered here.  For the canonical cycle basis we compute the exact size-dependent threshold
\[
\delta_n=1-\frac{1}{2\cos^2\!\Big(\frac{\pi}{4n}\rho(n)\Big)},
\quad
\rho(n)\in\{0,1,2,4\}\ \text{determined by } n\bmod 8,
\]
which improves the previously used \(1/2\) bound if and only if \(8\nmid n\), and we prove sharpness for that template.  We then introduce an optimally phase-aligned cycle basis for which
\[
\delta_n^{\rm (ph)}=
\begin{cases}
...
\end{cases}
\]
so \(\delta_n^{\rm (ph)}<1/2\) for every \(n\ge3\).  In Hadamard orders, including Sylvester--Walsh and Paley families, the coherence is optimal, \(M(Q)=1\), and every strict Sule\u{\i}manova list is realised.  The numerical part of the paper gives reproducible tests of the certificates, floating-point residuals, computational cost, Monte-Carlo success probabilities, and two application-oriented experiments: design of reversible Markov chains with prescribed mixing rates and average-consensus iterations with certified non-negative weights.
\end{abstract}
...
\item prove a general `coherence $\Rightarrow$ sufficient $\delta$' lemma and apply it to Walsh--Hadamard bases from elementary abelian $2$-groups and to Hadamard orders (via Paley), obtaining \emph{$\delta=0$ in those orders only}. Here `$\delta=0$' means the \emph{strict} condition $1+\sum\lambda_j>0$ suffices; this does not contradict obstructions on the boundary $1+\sum\lambda_j=0$ (\emph{e.g.}, $(1,0,\dots,0,-1)$ in odd dimensions);
\item give a numerical linear algebra implementation path: dense Schur construction, coherence-based certification, residual checks, timing experiments, Monte-Carlo stress tests, and two application tests in reversible Markov chains and average consensus.
\end{itemize}


\section{Structured Schur realisation and numerical certification}\label{sec:algorithms}

The constructive part of the SDIEP can be viewed as an inverse eigensolver with a non-negativity certificate.  Instead of starting from a matrix and computing its eigenvalues, we start from the desired eigenvalues and choose a structured orthogonal basis in which the Perron vector is fixed.  The numerical task is then to construct
\[
        P=Q\Lambda Q^{\top},\qquad Q^{\top}Q=I,
        \qquad q_0=\frac{\mathbf 1}{\sqrt n},
\]
and to certify that the entries of $P$ are non-negative.  The following algorithm is the implementation used in the experiments of Section~\ref{sec:numerics}.

\begin{alg}[Structured Schur realisation with coherence certificate]\label{alg:schur}
Given a list $\sigma=(1,\lambda_2,\ldots,\lambda_n)$ with $\lambda_j\le0$, a structured basis family, and a numerical tolerance $\tau$:
\begin{enumerate}[leftmargin=*,itemsep=2pt]
\item Construct an orthogonal matrix $Q=[q_0,\ldots,q_{n-1}]$ with $q_0=\mathbf 1/\sqrt n$.
\item Compute, or evaluate from a closed formula, the coherence
\[
        M(Q)=n\max_{1\le j\le n-1}\|q_j\|_{\infty}^2.
\]
\item Let $T=1+\sum_{j=2}^n\lambda_j$.  If $T>1-1/M(Q)$, record the analytic certificate
\[
        \min_{k,\ell}P_{k\ell}\ge \frac{1}{n}\bigl(1+M(Q)(T-1)\bigr)>0.
\]
\item Form $Y=Q\Lambda$ by diagonal scaling and then $P=YQ^{\top}$ by a dense matrix--matrix product.
\item Report the diagnostic quantities
\[
\|P\mathbf 1-\mathbf 1\|_{\infty},\quad
\|P-P^{\top}\|_{F},\quad
\min_{k,\ell}P_{k\ell},\quad
\max_i |\mu_i(P)-\lambda_i|,
\]
where the last residual compares sorted spectra.  Accept the numerical realisation if the analytic certificate holds or, in exploratory tests, if $\min_{k,\ell}P_{k\ell}\ge-\tau$.
\end{enumerate}
\end{alg}

For an explicitly stored dense basis, the diagonal scaling costs $O(n^2)$ operations and the product $YQ^{\top}$ costs $O(n^3)$ operations.  These costs are appropriate when the full transition or averaging matrix is needed.  If only products $Px$ are required, the same factorisation gives
\[
        Px=Q\bigl(\Lambda(Q^{\top}x)\bigr),
\]
so the cost is determined by fast applications of $Q$ and $Q^{\top}$: Walsh--Hadamard bases allow fast Walsh--Hadamard transforms, and the cycle bases can be applied through Fourier-type transforms after passing through the corresponding real representation.  Thus the constructions below are not merely existence proofs; they provide structured matrix generators and matrix-free linear operators with verifiable stochasticity and spectral residuals.
...
\section{Phase-optimised cycle bases}\label{sec:phase}
...
P=Q^{(\mathrm{ph})}\Lambda\big(Q^{(\mathrm{ph})}\big)^{\!\top},\\
& P\ \text{is symmetric, doubly stochastic, and entrywise non-negative}.
\end{aligned}
\]
\end{cor}
...
The cycle basis is the standard Fourier basis for the cyclic group $\mathbb{Z}_n$. To achieve the minimal coherence $M=1$ with a real orthogonal basis, we turn instead to the elementary abelian $2$-group. General finite abelian $2$-groups may have complex characters; the real $\{\pm1\}$ character table used below is the one for the exponent-two group $G=(\mathbb{Z}/2\mathbb{Z})^d$. Its elements are binary vectors $\mathbf{x}=(x_1,\dots,x_d)$, and the group operation is vector addition modulo $2$ (bitwise XOR).

The natural analogue of the Fourier basis on this group is the basis of its \emph{real characters}. For each indexing vector $\mathbf{a}\in G$, there is a corresponding character $\chi_{\mathbf{a}}:G\to\{\pm1\}$ defined by the dot product:
\[
...
\begin{thm}[Walsh--Hadamard bases from elementary abelian $2$-groups]\label{thm:hypercube}
...
We complement the theoretical analysis with numerical experiments aimed at numerical linear algebra usage: construction of structured bases, certification of entrywise non-negativity, residual checks, timing of dense kernels, Monte-Carlo stress testing, and application of the resulting matrices as Markov-chain and consensus operators. All computations were performed in Python~3.12 with NumPy~1.26.4~\cite{numpy}; the plotting used Matplotlib~\cite{matplotlib}. Complete code, data files, and exact reproduction instructions are available at \url{https://github.com/tomaszkania/sdiep-schur}, and the supplementary archive contains the scripts and CSV files used for the application tests below. We test entrywise non-negativity with the tolerance $\min_{k,\ell} P(\Lambda)_{k\ell}\ge -10^{-12}$, which is conservative relative to round-off.

\subsection{Methodology}
For each dimension $n$ we construct orthogonal bases $Q$ with Perron column $q_0=\mathbf{1}/\sqrt n$ of four types: (i) canonical cycle (Section~\ref{sec:cycle}); (ii) phase-optimised cycle (Section~\ref{sec:phase}); (iii) Walsh--Hadamard (powers of two); and (iv) Paley type~I Hadamard (orders $q+1$ with $q\equiv3\pmod4$ prime). 
...
\caption{Empirical realisation probability $\mathbb{P}[P(\Lambda)\ge 0]$ as a function of the trace sum $T=1+\sum_{j\ge 2}\lambda_j$ for selected dimensions. Each block corresponds to one dimension ($n=8,12,16,24$), and within each block we compare the basis families implemented in the repository \url{https://github.com/tomaszkania/sdiep-schur}. The main numerical message is a typical-case/worst-case distinction: the analytic quantities $\delta_n$ and $\delta_n^{(\mathrm{ph})}$ are worst-case sufficient thresholds, whereas for Dirichlet-sampled Sule\u{\i}manova lists the Schur template succeeds with high probability already below those values. In Hadamard orders (visible here at $n=16$), the success rate is numerically $1$ for all tested $T>0$, in agreement with Lemma~\ref{lem:coh}.}
\label{fig:success}
\end{figure}



\subsection{Application: reversible Markov chains with prescribed mixing rates}
A symmetric doubly stochastic matrix is the transition matrix of a reversible Markov chain with uniform stationary distribution.  Consequently, the Schur construction can be used as a numerical design method: prescribe the non-Perron eigenvalues, certify non-negativity, and obtain a Markov chain whose convergence is controlled by the second-largest eigenvalue modulus (SLEM); see, for instance, the standard spectral viewpoint in Markov-chain mixing theory~\cite{LPW17}.

We tested this design problem in the Hadamard order $n=16$.  For a fixed trace sum $T=0.05$ and target rates $r\in\{0.25,0.50,0.75,0.90\}$ we used
\[
\lambda_2=-r,\qquad
\lambda_3=\cdots=\lambda_{16}=-\frac{1-T-r}{14}.
\]
The Hadamard certificate gives $\min_{k,\ell}P_{k\ell}\ge T/n=0.003125$ for every tested $r$.  Figure~\ref{fig:markovmixing} shows the total-variation distance from uniform for a point-mass initial distribution.  The observed iteration counts increase with the prescribed SLEM, as expected from the spectral construction.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.82\textwidth]{fig_exp6_markov_mixing.pdf}
  \caption{Reversible Markov-chain design in the Hadamard order $n=16$.  Each curve uses a spectrum with trace sum $T=0.05$ and prescribed SLEM $r$.  The constructed transition matrices are symmetric, doubly stochastic, and entrywise non-negative by the $M(Q)=1$ certificate.  The total-variation distance from the uniform stationary distribution decays at a rate governed by the chosen non-Perron spectrum.}
  \label{fig:markovmixing}
\end{figure}

\subsection{Application: average consensus with certified non-negative weights}
The same matrices define linear average-consensus iterations
\[
        x^{(m+1)}=P x^{(m)},
\]
where symmetry and double stochasticity preserve the arithmetic mean and entrywise non-negativity makes every update a convex averaging step.  Such iterations are a standard numerical linear algebra model in distributed averaging and networked multi-agent systems~\cite{XiaoBoyd04,OlfatiSaber07}.

This test illustrates why the phase-optimised cycle basis is useful beyond a marginal theoretical improvement.  Take $n=24$, so $8\mid n$ and the canonical cycle certificate has threshold $1/2$.  We prescribe the one-spike spectrum
\[
        (1,-0.505,0,\ldots,0),\qquad T=0.495.
\]
Here $T$ is below the canonical cycle threshold, but above the phase threshold $\delta_{24}^{(\mathrm{ph})}\approx0.491334$.  The canonical cycle construction with this same spectrum has a negative entry $-4.17\cdot10^{-4}$, whereas the phase-optimised construction is certified and has minimum entry $3.00\cdot10^{-4}$.  Figure~\ref{fig:consensus} shows the resulting consensus error for a random initial vector.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.82\textwidth]{fig_exp7_consensus.pdf}
  \caption{Average-consensus test for the phase-optimised cycle basis at $n=24$ with spectrum $(1,-0.505,0,\ldots,0)$ and trace sum $T=0.495$.  The phase basis gives a valid non-negative averaging matrix even though the same trace sum lies below the canonical cycle threshold.  The relative disagreement norm decays according to the prescribed SLEM $0.505$.}
  \label{fig:consensus}
\end{figure}

\begin{table}[t]
\centering
\caption{Application diagnostics.  The Markov rows use the Hadamard basis at $n=16$ and tolerance $10^{-3}$ for total variation.  The consensus rows use $n=24$ and tolerance $10^{-6}$ for the relative disagreement norm; the canonical row is included as a failed non-negativity comparator for the same spectrum.  Row-sum and symmetry residuals in the successful rows are at round-off level.}
\label{tab:application_diagnostics}
\begin{tabular}{lrrrrrr}
\toprule
Case & $n$ & $T$ & SLEM & $\min P_{k\ell}$ & spectral residual & iter. \\
\midrule
Hadamard, $r=0.25$ & 16 & 0.050 & 0.250 & $3.125\cdot10^{-3}$ & $4.72\cdot10^{-16}$ & 5 \\
Hadamard, $r=0.50$ & 16 & 0.050 & 0.500 & $3.125\cdot10^{-3}$ & $5.55\cdot10^{-16}$ & 9 \\
Hadamard, $r=0.75$ & 16 & 0.050 & 0.750 & $3.125\cdot10^{-3}$ & $2.22\cdot10^{-16}$ & 22 \\
Hadamard, $r=0.90$ & 16 & 0.050 & 0.900 & $3.125\cdot10^{-3}$ & $2.22\cdot10^{-16}$ & 59 \\
Phase cycle & 24 & 0.495 & 0.505 & $3.00\cdot10^{-4}$ & $2.67\cdot10^{-16}$ & 17 \\
Canonical cycle & 24 & 0.495 & 0.505 & $-4.17\cdot10^{-4}$ & $2.22\cdot10^{-16}$ & -- \\
\bottomrule
\end{tabular}
\end{table}
...
Evaluating $P(\Lambda)=Q\Lambda Q^\top$ as a dense matrix uses Level-3 BLAS operations: forming $Q\Lambda$ costs $O(n^2)$ flops (diagonal scaling), and the product $(Q\Lambda)Q^\top$ costs $O(n^3)$ flops using standard dense matrix multiplication. For moderate sizes (e.g.\ $n\le 128$) this is inexpensive on a laptop CPU, and for repeated evaluations the structured bases (FFT/FWHT) allow significantly faster matrix--vector products without forming $P(\Lambda)$ explicitly. Figure~\ref{fig:timing} summarises the measured construction and dense evaluation times in the tested range.

\begin{figure}[h!]
  \centering
  \includegraphics[width=0.82\textwidth]{fig_exp4_timing.pdf}
  \caption{Measured construction and dense Schur-evaluation times for the implemented basis families. The timing experiment confirms the expected dense behaviour: basis construction is quadratic in storage, while forming the full matrix $P=Q\Lambda Q^{\top}$ is dominated by a dense matrix--matrix product. Walsh--Hadamard bases are especially cheap to construct recursively and can also be applied matrix-free by the fast Walsh--Hadamard transform.}
  \label{fig:timing}
\end{figure}
...
\item The Markov-chain and consensus tests show how the constructions can be used as numerical design tools: one prescribes spectral behaviour, receives a certified stochastic matrix, and verifies the row-sum, symmetry, spectral, and non-negativity residuals.
\end{itemize}

All code and data to reproduce Figures~\ref{fig:coherence}--\ref{fig:timing}, including the application experiments, are available at \url{https://github.com/tomaszkania/sdiep-schur}.  The supplementary archive contains the CSV files and the script \texttt{nla\_application\_experiments.py} used for Figures~\ref{fig:markovmixing} and~\ref{fig:consensus}.
...
\newblock Second edition, American Mathematical Society, Providence, RI, 2017. With contributions by E.~L. Wilmer.

\bibitem{OlfatiSaber07}
...
\newblock \emph{Systems \& Control Letters}, 53(1):65--78, 2004.
